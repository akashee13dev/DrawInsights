...
from constants import GEMNI_API_KEY

genai.configure(api_key=GEMNI_API_KEY)

def parse_response(response):
    text = response.text.strip()
    # Remove markdown code block markers (```json or ```), if present
    # This regex removes any ```xxx at the start and ``` at the end
    cleaned_text = re.sub(r"^```[a-zA-Z]*\n|```$", "", text, flags=re.MULTILINE).strip()
    print("Cleaned text:", cleaned_text)
    try:
        # Use ast.literal_eval because the data is python-like (single quotes, True/False)
        parsed = ast.literal_eval(cleaned_text)
    except Exception as e:
        print(f"Error parsing cleaned text: {e}")
        parsed = []
    return parsed
